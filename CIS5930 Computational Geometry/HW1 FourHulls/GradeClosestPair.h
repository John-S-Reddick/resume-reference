...
void evaluate() {
    ...
    for(unsigned n = 1; n <= 20; ++n) {
        ...
        for(unsigned i = 0; i < 5; ++i) {
            ...
            bool isCorrect = (closestPairFast.first == closestPairSlow.first && closestPairFast.second == closestPairSlow.second) ||
                             (closestPairFast.first == closestPairSlow.second && closestPairFast.second == closestPairSlow.first);
    ...
    for(unsigned i = 0; i < 20; ++i)
        out << (i+1)*1000 << " " << runtimes[i][0] << " " << runtimes[i][1] << std::endl;
    out.close();
